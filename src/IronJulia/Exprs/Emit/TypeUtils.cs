...
public static class TypeUtils
{
    ...
    public static (Type? ParameterCollectionType, Type? ParameterElementType) GetParameterCollectionInformation(ParameterInfo possibleParameterInfo) {
        if (possibleParameterInfo.IsDefined(typeof(ParamArrayAttribute), true)) {
            return (possibleParameterInfo.ParameterType, possibleParameterInfo.ParameterType.GetElementType());
        }
        if (possibleParameterInfo.IsDefined(typeof(ParamCollectionAttribute), true)) {
            return (possibleParameterInfo.ParameterType, TryGetCollectionElementType(possibleParameterInfo.ParameterType));
        }
        return (null, null);
    }

    public static Type? TryGetCollectionElementType(Type collectionType) {
        if (collectionType.IsArray)
            return collectionType;
        foreach (var bt in collectionType.GetInterfaces()) {
            if (!bt.IsGenericType || bt.GetGenericTypeDefinition() != typeof(ICollection<>)) continue;
            return bt.GetGenericArguments()[0];
        }
        return null;
    }
    
    public static Type? TryGetCollectionGenericType(Type collectionType) {
        if (collectionType.IsArray)
            return collectionType;
        foreach (var bt in collectionType.GetInterfaces()) {
            if (!bt.IsGenericType || bt.GetGenericTypeDefinition() != typeof(ICollection<>)) continue;
            return bt.GetGenericTypeDefinition();
        }
        return null;
    }
    ...
    public static Type LiftPrimitiveOrThrow(this Type type) {
        ...
        throw new NotSupportedException("LiftingInExpressionRequiresDynamicCode " + type);
    }
    ...
    public static bool IsOrContainsReferences(Type t) =>
        (bool) CachedReflectionInfo.RuntimeHelper_isOrContainsReferences.MakeGenericMethod(t).Invoke(null, [])!;
    ...
    public static MethodInfo? FindConversionOperator(MethodInfo[] methods, Type? typeFrom, Type? typeTo, bool onlyImplicit) {
        foreach (MethodInfo mi in methods) {
            if ((mi.Name == "op_Implicit" || (!onlyImplicit && mi.Name == "op_Explicit")) && AreEquivalent(mi.ReturnType, typeTo)) {
    ...
    public static bool IsImplicitReferenceConversion(Type source, Type destination) => destination.IsAssignableFrom(source);
    public static bool IsImplicitBoxingConversion(Type source, Type destination) =>
    ...
    public static void ValidateType(Type type, string? paramName, bool allowByRef, bool allowPointer) {
        if (ValidateType(type, paramName, -1)) {
            if (!allowByRef && type.IsByRef) {
                throw new Exception("TypeMustNotBeByRef" + type);
            }

            if (!allowPointer && type.IsPointer) {
                throw new Exception("TypeMustNotBePointer " + type); 
            }
        }
    }
    ...
    public static bool ValidateType(Type type, string? paramName, int index) {
        ...
        if (type.ContainsGenericParameters) {
            throw type.IsGenericTypeDefinition
                ? new Exception($"{paramName}::${type} Is Generic")
                : new Exception($"{paramName}::${type} Contains Generic Parameters");
        }
    ...
    public static bool IsUnsigned(this Type type) => IsUnsigned(Type.GetTypeCode(GetNonNullableType(type)));
    ...
    public static bool IsFloatingPoint(this Type type) => IsFloatingPoint(Type.GetTypeCode(GetNonNullableType(type)));

    public static bool IsFloatingPoint(this TypeCode typeCode) {
    ...
    public static TypeCode GetTypeCode(this Type t) => Type.GetTypeCode(t);
}